...
private void fnSiengePayload_ExecuteCode(object sender, System.EventArgs args)
{
    try
    {
		...
		//this.StatusIntegracao = "PAYLOAD GERADO: " + jsonPayload;
        //throw new System.Exception(this.StatusIntegracao); // FORÇA O DISPLAY debug
...
private string fnSiengeBuildPayload()
{
    try 
    {
        ...
        if (dtHead.Rows.Count == 0)
            throw new System.Exception("Dados de cabeçalho (Header) não encontrados no SQL.");

        var rowHead = dtHead.Rows[0];
        var listBudgetCategories = new System.Collections.Generic.List<object>();

        decimal somaPercentualAcumulado = 0;
        int totalLinhas = dtDetail.Rows.Count;

        if (totalLinhas == 0)
            throw new System.Exception("Dados de detalhe (Rateio) não encontrados no SQL.");

        for (int i = 0; i < totalLinhas; i++)
        {
            var row = dtDetail.Rows[i];
            
            // Proteção contra valores nulos vindos do SQL
            decimal valorBanco = row["PERCENTAGE"] == System.DBNull.Value ? 0 : System.Convert.ToDecimal(row["PERCENTAGE"]);
            decimal percentualAtual = System.Math.Round(valorBanco, 4);

            if (i == totalLinhas - 1)
            {
                percentualAtual = 100.0m - somaPercentualAcumulado;
                if (percentualAtual < 0) percentualAtual = 0; 
            }
            else
            {
                somaPercentualAcumulado += percentualAtual;
            }
    ...
    {
        // Aqui pegamos o erro real interno para não vir a mensagem genérica do RM
        throw new System.Exception("Erro na montagem do JSON: " + ex.Message);
    }
}
// =========================================================================
